...
resolver serves stale grants (learned v3-0/v3-c; applies to v3-f prune + v3-b/v3-a removals). · G10 `acl_entries`
has **no provenance column** (no `role_id`) — a `(holder, key, scope)` cell is ONE shared row, so manage it via one
mechanism (role baseline **or** card editor **or** projector), never two; removing one removes the shared row
(learned v3-d/ADR-0084; v3-b/v3-a must design around it).
