...
> Goal: produce a single **ready-to-upload `hearth-release.zip`** the owner can drop onto a shared host (no
...
```
Build the deployable hearth-release.zip for shared-host upload. No new features.
...
BUILD (in the Docker php:8.3 env + host Node):
  ...
  4. Zip the app to hearth-release.zip. INCLUDE: vendor/, public/build/, app, bootstrap (dirs only),
     ...
     .env.example. EXCLUDE (critical):
       ...
       database/*.sqlite
       hearth-release.zip          (don't zip the output into itself)
...
DELIVER:
  • Write the artifact to D:\Forum\hearth-release.zip (repo root, so the owner finds it in their folder).
  • Add `/hearth-release.zip` to .gitignore (it's a build artifact — do NOT commit the binary).
...
The owner downloads `hearth-release.zip` from their `D:\Forum` folder and uploads it to each shared host per
