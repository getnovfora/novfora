...
2026-06-02). Run `git status` to confirm a clean tree before you start. The empty HearthBB/
dir is a stray — reconcile it (use as the app dir or delete it); the spike scaffolds into a
separate scratch dir `hearth-spike/`, and M0 (later) builds the app at the repo root per the
...
CONSTRAINTS: throwaway spike, keep it in hearth-spike/ until GO; no production code at the repo
