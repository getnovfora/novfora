...
- **Hosting floor:** **Modern PHP host — PHP 8.3+, Composer, SSH, MySQL/MariaDB, cron.** Assume the
...
- **Stack (validated & accepted at the Phase 0 gate — ADR-0001/0002):**
  **Laravel 13 (PHP 8.3) + Livewire 4 + Alpine.js + Blade**, server-rendered for SEO, on
...
- **Phase 1 — Core MVP:** Laravel + Livewire skeleton; **service-tier detection + driver abstraction**
  ...
  **WYSIWYG editor** (+ Markdown mode + BBCode import) with drag-drop uploads; email notifications;
  ...
  + reversible-migration baseline. Must run on a PHP 8.3 + MySQL + cron host.
