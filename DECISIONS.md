...
2. **Role baseline(s).** For EACH of the source's `role_assignments` rows, re-run `RoleExpander::assign(role, 'group', clone.id, scope)` — the expander writes both the `role_assignments` row AND its expanded `acl_entries`, so the two stay in sync (the role-derived rows are NOT hand-copied).
3. **Direct `acl_entries`.** Then copy **EVERY** one of the source's own `acl_entries` rows verbatim (`value` + `expires_at`) across **every** scope (global + per-forum + per-club), upserting onto the clone. Because this runs **after** step 2, a card-editor override that differs from the role wins on the clone *exactly as on the source* — and since provenance can't be discriminated, **copying the physical rows IS the provably-exact rule**: for every `(key, scope)` the clone's physical row equals the source's. ALLOW stays ALLOW, **NEVER stays NEVER**, a `no` (no row) is simply not copied → inherit.
...
- **Group clone** — `manage-permissions` capability + the **rank guard** (you can't clone a group you couldn't edit: `isAdmin || rankPriority > source.priority`) + the **admin-tier fence** (a non-admin can't clone a group bearing an Administration-cluster key); **only `custom` groups are cloneable** (`system`/`trust` refused — their preset is their seeded identity).
...
**Alternatives considered.** (a) Copy only "manual override" `acl_entries` and let the expander supply the role keys — rejected: provenance is undiscriminable (`acl_entries` has no `role_id`), so "manual only" can't be computed reliably; copying all physical rows after the expansion is strictly exact and simpler. (b) A schema `role_id` column on `acl_entries` to track provenance — rejected (large engine change for a copy feature; out of scope). (c) Auto-assign the role clone to the source's groups — rejected (silently adds a second baseline to live groups). (d) Clone membership too — rejected (a clone is a permission template; carrying members would duplicate co-ownership/primary flags and surprise operators).
