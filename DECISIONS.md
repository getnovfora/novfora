...
**Context.** v3-a…v3-f built the *machinery* of admin & permission management. v3-g is the **display** capstone: surface who is staff at a glance (a small role marker on posts / profiles / the members directory) and a curated public **"The Team"** page. Deliberately the **only non-correctness-load-bearing slice** — it NEVER reads or writes `acl_entries`, runs no resolver, bumps no `AclVersion`, and has no concurrency/untrusted-input seam (Sonnet-forward; ultracode started at the apex and downgraded once the single reasoning step — the role resolver — was settled). Additive + reversible; branch `claude/acp-v3-g` off `main`.

**Decision — the role resolver (step 1, the one reasoning step).** `User::staffRole(): ?string` returns a CANONICAL role KEY (not a label) — one of `co_owner` / `administrator` / `moderator` / `forum_moderator`, or null — resolved highest-first: an `admins`-group member carrying the co-owner flag → `co_owner`, else `administrator`; a `moderators`-group member → `moderator`; otherwise a per-user per-forum moderator assignment (v3-b) → `forum_moderator`; else null. Returning a key (label deferred to `__('forum.role_'.$key)`) keeps presentation out of the model and lets the roster group + order by role. Reuses `isAdmin()`/`isStaff()` (the already-loaded `groups` relation), the co-owner check (`AdminCoOwnerService::isCoOwner` — the ONLY extra DB touch, and only for an admin), and `moderator_assignments`. Memoized per instance. Two display-only companions read the loaded groups (no query): `staffTitle()` (the highest-priority staff group's optional `staff_title` override) and `showsStaffIcon()`.

**The one performance seam (NOT an apex seam, but it trips a gate).** The topic page has a `HotPathQuery` budget that forbids a per-post N+1. The naive `forum_moderator` check is a per-author `moderator_assignments` EXISTS — an N+1 across distinct posters. Resolved by exposing the assignments as an eager-loadable `User::moderatorAssignments()` relation (`hasMany(holder_id)` with a baked-in `holder_type='user'`) and eager-loading `author.moderatorAssignments` in `TopicController` (+ `moderatorAssignments` on the members-directory SFC): ONE board-wide IN query, never per-post. The topic ceiling moved `<41` → `<42` to absorb that single constant (mirroring the v3-e nav-EXISTS bump) — verified by the gate at a 16-distinct-author cold render.

**Decision — flair component + the four surfaces (step 2).** `<x-ui.staff-flair :user>` renders the role badge (label = `staffTitle()` override ?? `forum.role_<key>`, optional `groups.show_staff_icon` icon), gated by the global `members.staff_flair_show_badge` setting; renders nothing for a guest/deleted/non-staff user. Slotted into the post author block (replacing the old inline `role_admin`/`role_moderator` ternary — the staff-status truthiness that protects a staff member's post from ignore-collapse now reads `staffRole()`), the profile hero, and the members-directory card. The **online list is intentionally deferred** (the spec marks it optional): its Livewire state is primitive `{id,username}` arrays, so a `:user`-model component is a poor architectural fit.

**Decision — settings + group columns (step 3).** Two pure-DB settings in `SettingsRegistry` (group `members`): `members.staff_flair_show_badge` (bool, default **true**) + `members.staff_roster_enabled` (bool, default **false** — opt-in disclosure). One additive, reversible migration adds three DISPLAY-ONLY `groups` columns: `show_on_staff_page` (bool, default false — **seeded true on `admins` + `moderators`**: the migration backfills existing installs, `GroupSeeder` covers fresh ones), `show_staff_icon` (bool, default false), `staff_title` (nullable — the only "custom title" surface; there are NO per-user titles, a v3-g non-goal). All three added to `Group::$fillable` (cosmetic, never feed resolution) + bool casts. down() drops all three (G3).

**Decision — "The Team" roster (step 4).** `/staff` → `members.staff` (an `@extends` wrapper) → the `⚡community.staff-roster` SFC. Gated by `members.staff_roster_enabled` at BOTH the route (404 when off, mirroring the directory's route-guard) AND the SFC mount/data method (Livewire bypasses route middleware). Candidates = ACTIVE members of `show_on_staff_page` groups ∪ active per-user forum-moderators; each is bucketed by `staffRole()` (self-dedup) and rendered in the fixed order Co-owners → Administrators → Moderators → Forum moderators. A flagged-group member whose `staffRole()` is null is omitted (the four canonical buckets are what the page shows); a non-flagged group never leaks.

**Decision — ACP toggle + nav (step 5).** `⚡admin.settings.staff-flair` (the two toggles, self-guarding in mount + save like every admin SFC) + an `@extends` wrapper at route `admin.members.staff-flair` + an `AdminNavigation` Members sub-page `[staff_flair, …, shield]` (Route::has-gated) + the `admin.nav.staff_flair` string.

**Consequences.** One additive reversible migration (G3; gate exercises apply+rollback+re-apply). **No `acl_entries` / resolver / `AclVersion` change** (the slice is display-only). The topic hot-path budget moved by one constant (documented). **This completes the ACP v3 program (ADR-0080): v3-0, v3-h, v3-c, v3-e, v3-d, v3-b, v3-a, v3-f, v3-g all shipped.**

**Verification.** `StaffFlairTest` (staffRole per role incl. the admin-over-forum-mod precedence + null member; the component shows the label / suppresses when the setting is off / honours a `staff_title` override / renders on a topic page; the ACP page gate + the two-toggle round-trip) + `StaffRosterTest` (404 when disabled; grouped listing; no non-flagged-group leak; forum-mod bucket; banned excluded; co-owner/administrator split; unflagging a system group drops it) — 19 cases. Full suite **1864 pass / 1 skip**. Gate: `pest` · `pint` · `phpstan` L max · `migrate` apply+rollback+re-apply.

**Alternatives considered.** (a) Derive the `forum_moderator` flair with a per-user EXISTS in the resolver — rejected (per-post N+1; the eager-loaded relation keeps it O(1)). (b) Reuse the earned-badge system for flair — rejected (flair is LIVE/group-derived, not awarded; a v3-g non-goal). (c) Per-user custom titles — rejected (only a per-group `staff_title`, "only if trivial"). (d) Read the co-owner flag off the loaded `group_user.is_co_owner` pivot instead of `AdminCoOwnerService::isCoOwner` — declined (reuse the single source of truth; co-owner resolution only fires for an admin, never on the regular-member hot path). (e) Force the flair into the online-list widget — declined (its primitive-array Livewire state; the spec marks it optional). (f) Bucket the roster by raw group instead of role — rejected (co-owners must split from administrators within the admins group; the order is by role).
### ADR-0089 — Simple-mode permissions: the layman capability→key mapping over the existing card editor (2026-06-21)
**Status: Accepted — owner-authorized design-first build; the capability→key mapping is FLAGGED FOR OWNER REVIEW. A friendlier WRITE surface, NOT an engine change.** *(ADR-0088 is reserved by the parallel `claude/acp-v3-g` staff-flair branch built the same day; this branch — `claude/simple-mode-perms` — is independent off `main`, so it takes the next free number 0089.)*

**Context.** The v3-c card editor (ADR-0082) exposes a 42-key Yes/No/Never grid per group at global/forum/club scope. That is the right tool for power users but overwhelming for a non-technical operator. Simple mode adds **~7 plain-language capability toggles** that write the SAME `acl_entries` through the SAME write primitive (`GroupPermissionEditor::set`), with an **"Advanced"** escape to the card editor. **No engine/resolver/catalog/`GroupPermissionEditor` change** (G1) — simple mode only BATCHES existing `set()` calls behind one transaction + one audit entry. Because a wrong capability→key mapping silently mis-grants, the mapping is the **correctness core** and is treated as apex-adjacent (the permission editor is an apex surface): this ADR ships FIRST and pins the mapping, the scope handling, and the NEVER/trust conflict handling.

**Decision — the capability→key mapping (the load-bearing core).** Seven capabilities, each a bundle of catalog keys. The single source of truth is `App\Permissions\CapabilityMap` (a pure, Larastan-clean value class — NOT a new engine path); the SFC and the tests both read it.

| Capability | Keys (bundle) | Bundle scope_kind |
|---|---|---|
| **read_reply** — Read & reply | `forum.view`, `post.create`, `post.edit.own`, `post.delete.own` | all `forum` |
| **start_topics** — Start new topics | `topic.create` | `forum` |
| **post_media** — Post links & images | `post.links`, `post.images`, `attachment.create` | all `forum` |
| **react_vote** — React & vote | `react.create`, `poll.vote` | all `forum` |
| **polls_tags** — Create polls & tags | `poll.create`, `tag.apply`, `tag.create` | MIXED (`tag.create` is `global`) |
| **follow** — Follow members | `follow.create`, `follow.delete` | all `global` |
| **pm** — Send private messages | `pm.send` | `global` |

Each bundle groups keys that an operator thinks of as ONE capability and that share an enforcement story: read_reply = the baseline "participate" set; post_media + polls_tags include the trust-gated extras (`post.links`/`post.images`/`tag.create` carry the TL0 hard NEVER, `poll.create` the soft gate) so the operator sees them where they expect them; follow/pm are the global social keys. `follow.delete`/`tag.apply` ride with their create sibling so "off" doesn't strand a half-capability (undo-your-own-follow and apply-existing-tag are ungated participation and stay granted via the member preset regardless — simple mode writes them ON together for clarity, OFF deletes only the group's own rows).

**Decision — pin 2: mixed `scope_kind` (the silently-inert-write trap).** A key is *settable at a scope* iff the card editor would expose it there (`scope_kind='forum'` → global/forum/club; `scope_kind='global'` → global only). **A capability is shown at a scope ONLY when EVERY key in it is settable there** (the kickoff's recommended option — never write a silently-inert row). Consequence: **global** shows all 7; **forum & club** show only the four all-`forum` capabilities {read_reply, start_topics, post_media, react_vote}. `polls_tags` (mixed — `tag.create` is global), `follow`, and `pm` are **global-only** in simple mode; per-forum poll/tag-apply nuance is an Advanced-mode task. `CapabilityMap::for(Scope)` is the single gate; the SFC renders only `for($scope)` and `setCapability` re-asserts it (422 otherwise), so a crafted Livewire call can never write an inert/out-of-scope row. *(Alternative rejected: split each key to its own catalog scope — more surface + a confusing partial-capability at forum scope; fidelity to the proposed bundle + the recommended rule wins, and Advanced covers the gap.)*

**Decision — pin 3: the "toggle vs a NEVER / trust gate" case (don't lie + don't silently mis-grant).** Hard anti-spam NEVERs live as `acl_entries` on the `tl0` group at global scope (`TrustGateSeeder`: `post.links`/`post.images`/`pm.send`/`tag.create` = NEVER; `attachment.create`/`poll.create`/`follow.create` = soft `no`). NEVER is absolute. So: **a capability is "restricted" when the group being edited carries an explicit NEVER for ANY of its keys at the current scope OR (for a non-global scope) at global scope.** A restricted capability renders a plain note ("Restricted by a trust-level or Never rule — manage in Advanced") AND its toggle is **locked**. This is stricter than the kickoff's "note next to a still-green toggle," chosen deliberately: a layman must not be able to lift a hard anti-spam NEVER through a friendly switch (the exact mis-grant we're avoiding) — the card editor stays the only place to change a NEVER. Soft `no` gates are NOT restricted (they are admin-liftable by design, per the catalog), so toggling them ON grants normally. `setCapability` re-asserts the not-restricted invariant (403 backstop) so a crafted call can't lift a NEVER either.

**Decision — pin 4 + write semantics.** **Excluded from simple mode entirely (Advanced-only):** every Administration-tier key (`admin.*`, `users.manage`, `groups.manage`, `prefix.manage`, `badge.manage`, `permissions.manage`, the nine `admin.<section>.access`), the Moderation cluster (`bans.manage`, `post.edit.any`, `post.delete.any`, `post.history.view`, `topic.moderate`), and `club.manage`. "Moderate" is deliberately NOT a simple toggle: silently granting `bans.manage`/`post.edit.any` is precisely the catastrophic mis-grant simple mode exists to avoid — moderation is an explicit, per-forum decision (v3-b) made in Advanced. `CapabilityMap` is asserted to contain NO excluded key (a test pins it). **Write semantics (locked):** toggle ON → `set(group,key,scope,'yes')` for each key; toggle OFF → `set(...,'no')` (delete → inherit). **Simple mode NEVER writes `'never'`.** A capability reads "on" iff every key is explicitly ALLOW for the group at the current scope. The whole bundle writes in ONE `DB::transaction` with ONE `acl.group.capability_set` audit entry. The existing **rank guard + per-key escalation fence + admin-recovery lockout guard** are reused unchanged (none fire in practice — no excluded key is in a bundle — but they remain the backstop).

**Decision — the SFC + the Simple/Advanced switch.** `resources/views/components/permissions/⚡group-simple-editor.blade.php` (`<livewire:permissions.group-simple-editor scope-type scope-id>`) mirrors the card editor's `#[Locked]` scope + `mount()`/per-action gate (manage-permissions for global/forum, `club.manage` for club) + rank guard. The three homes (`admin/group-permissions.blade.php`, `admin/forum-permissions.blade.php`, `clubs/edit.blade.php`) gain a **Simple / Advanced** segmented switch (a `?mode=` query param, default **Simple**, remembered) that conditionally embeds `⚡group-simple-editor` vs the existing `⚡group-editor`. i18n under `admin.perms.*` (G8): `mode_simple`/`mode_advanced`, `capabilities.<key>.label`/`.subtitle`, `restricted_note`, `simple_intro`.

**Consequences.** No migration; no engine/resolver/catalog/`GroupPermissionEditor` change (G1). The advanced card editor is untouched and remains authoritative. The capability mapping is FLAGGED for owner review (it encodes a product judgment about what a layman should be able to grant). If the owner wants a capability added/moved, it is a one-line `CapabilityMap` edit + a lang key — no engine work.

**Verification.** `SimpleModePermissionsTest`: each capability ON writes EXACTLY its keys as ALLOW at the right scope and OFF deletes them (→ inherit), proven through the PermissionInspector oracle (G4); simple mode never writes a `never` row; `CapabilityMap` contains no Administration/Moderation/`club.manage` key (the exclusion invariant); `for(scope)` omits global-only capabilities at forum/club scope (no inert write — a crafted out-of-scope `setCapability` → 422); a trust-NEVER capability (tl0 `post_media`) is locked + noted and `setCapability` refuses it (403, the NEVER survives); a soft-gate capability toggles normally; the rank guard + escalation fence still bite; the Simple/Advanced switch renders on all three homes and the advanced editor still works. Gate: `pest` · `pint` · `phpstan` L max · `migrate` (no-op; schema unchanged).

**Alternatives considered.** (a) A `never` toggle in simple mode — rejected (locked constraint; OFF = inherit, NEVER never). (b) Show a restricted capability as a still-interactive green toggle with only a note — rejected for the apex-adjacent surface; locking prevents a layman lifting a hard NEVER (Advanced is the escape hatch). (c) Split mixed-scope bundles so `tag.create`/`poll.create` show at forum scope — rejected (fidelity to the proposed 7-bundle model + the recommended skip rule; Advanced covers forum-scope tag/poll nuance). (d) A "Moderate" simple toggle bundling the moderation cluster — rejected (the exact silent mis-grant simple mode avoids; moderation is per-forum in Advanced/v3-b). (e) New `Role`/preset objects for capabilities — rejected (simple mode is a UI write layer over a group's own `acl_entries`, not role machinery).

### ADR-0090 — Group / role clone copy-semantics: an exact, non-escalating duplicate (2026-06-21)
**Status: Accepted — apex seam (writes `acl_entries`). Built on `claude/admin-perm-mgmt` (Batch 2026-06-21, Branch 1); FLAGGED for adversarial review on the Cowork side.**

**Context.** Operators rebuild near-identical groups/roles by hand. A clone feature must reproduce the source's **effective** permissions *exactly* — copying too little silently *removes* a grant; copying a NEVER as anything else silently *adds* one — and must not become an escalation path (a non-admin minting a group/role that grants more than they could grant by hand). `acl_entries` carries **no `role_id`** (ADR-0084 provenance caveat): a key that is both role-derived and card-editor-overridden is ONE physical row (last-writer-wins), so role-vs-manual provenance cannot be discriminated at the row level.

**Decision — `GroupManager::clone(Group $source): Group` (the load-bearing core), in ONE transaction.**
1. **New `groups` row.** Copy `color, description, priority, type, auto_promotion, membership_model, is_public, show_on_staff_page, show_staff_icon, staff_title`; `name = source." (copy)"`; a fresh unique slug (the existing `uniqueSlug` helper); `is_system = false`.
2. **Role baseline(s).** Copy the source's `role_assignments` **rows verbatim** (the assignment rows only — `role_id` + scope), **NOT** via the expander. They keep the clone role-managed (the edit form shows the role; a future role edit re-expands onto the clone too), reproducing the source's exact assignment set.
3. **Direct `acl_entries`.** Copy **EVERY** one of the source's own `acl_entries` rows verbatim (`value` + `expires_at`) across **every** scope (global + per-forum + per-club). This step **alone** makes the clone's effective permissions IDENTICAL to the source's — the resolver reads only `acl_entries`, so the row-for-row copy is exact by construction: for every `(key, scope)` the clone's physical row equals the source's. ALLOW stays ALLOW, **NEVER stays NEVER**, a `no` (no row) stays absent → inherit. **Re-expanding the role here would be WRONG** — it would resurrect any role key the source had since card-edited to `no` (a deleted row), silently *adding* a grant the source lacks (an escalation when that key is admin-tier; caught in adversarial review).
4. **`AclVersion` bump (G9).** The Eloquent `updateOrCreate` writes bump via the model event, but `clone()` **also bumps once explicitly** so the guarantee holds even for a zero-entry source — a resolve immediately after clone is never stale.
5. **Cache + audit.** `GroupDirectory::forgetEnabled()` when the clone is public; **one** `group.cloned` audit entry (source id → new id). A failure rolls the whole clone back.
6. **Never copied:** `group_user` membership, `is_co_owner`, `is_primary`/`is_primary_locked` — **a clone starts with ZERO members.**

**Decision — `RoleManager::clone(Role $source): Role`.** Copy the role row (`name." (copy)"`, fresh slug, `is_preset = false` even when cloning a read-only preset) and its `role_permissions` rows into a new role that is **UNASSIGNED** (no `role_assignments`). It is inert — it grants nothing until an operator assigns it, and assignment runs the existing escalation fence (`assertWithinCeiling`). Deliberately NOT auto-assigned to the source's groups (that would silently add a second baseline to live groups).

**Decision — no escalation.** Both clone actions live in their ACP SFCs and reuse the **same** guards as the editors they sit beside (the service is the actor-independent backstop, mirroring `GroupPermissionEditor`):
- **Group clone** — `manage-permissions` capability + the **rank guard** (you can't clone a group you couldn't edit: `isAdmin || rankPriority > source.priority`) + the **admin-tier fence** (a non-admin can't clone a group bearing an Administration-cluster key **either as a direct `acl_entries` grant OR via an assigned role's permission keys** — the fence inspects both, mirroring the role SFC, so a stripped-but-still-assigned admin-tier role can't be cloned through the gap); **only `custom` groups are cloneable** (`system`/`trust` refused — their preset is their seeded identity).
- **Role clone** — reuses `assertNotAdminTierRole` (a non-admin can't duplicate a role carrying an Administration-tier key).

**Consequences.** No engine/resolver/catalog/`GroupPermissionEditor`/`RoleExpander` change — clone reuses the existing primitives. No migration. The clone is a one-line-extensible primitive on the managers. Supersedes nothing.

**Verification.** `GroupCloneTest` proves through the PermissionInspector oracle: a cloned custom group with mixed ALLOW/NO/NEVER across global + forum resolves **identically** to the source for a placed member (NEVER preserved); a manual override that differs from the role wins on the clone; the `AclVersion` bumps; the clone has zero members; system/trust clones are refused; exactly one audit entry; and the SFC blocks a non-admin from cloning an admin-tier or higher-ranked group while allowing a normal outranked custom group. `RoleCloneTest` proves an exact, editable, unassigned copy (incl. cloning a preset) and the admin-tier fence. Gate: `pest` · `pint` · `phpstan`.

**Alternatives considered.** (a) Re-expand the source's roles onto the clone (then copy physical rows over the top) — **rejected after adversarial review**: re-expansion resurrects any role key the source had stripped to `no`, silently widening the clone (an escalation when the key is admin-tier). Copying the source's physical `acl_entries` rows + the `role_assignments` rows verbatim is strictly exact (the resolver reads only `acl_entries`) and avoids the trap. (b) A schema `role_id` column on `acl_entries` to track provenance — rejected (large engine change for a copy feature; out of scope). (c) Auto-assign the role clone to the source's groups — rejected (silently adds a second baseline to live groups). (d) Clone membership too — rejected (a clone is a permission template; carrying members would duplicate co-ownership/primary flags and surprise operators).

### ADR-0092 — Soften the trust-warning freeze: a live warning caps promotion at TL1 instead of pinning at TL0 (2026-06-21)
**Status: Accepted — owner-directed product change on `claude/post-approval-promotion` (Batch 2026-06-21, Branch 2 follow-up; updates PR #44). Moderation/trust layer — no `acl_entries` touch.**

**Context.** `TrustLevelManager::evaluate()` froze trust promotion at the member's CURRENT level whenever any live (sub-demotion) warning point existed. Combined with the new-user hold gate keying on **TL0 group membership**, a single live warning **pinned a long-time, active member in TL0 new-user moderation indefinitely** — the warning-freeze finding flagged in PR #44. The owner's call: a minor/expiring warning must not trap a member in moderation, but warnings should still gate *climbing higher*.

**Decision.** Split the freeze in `evaluate()` (ONLY the promotion-freeze behaviour changes):
- **Demotion unchanged** — live infraction points ≥ `demotion_points` still hard-demote to TL0.
- **Non-active status unchanged** — a `pending` / `suspended` account stays frozen entirely at its current level.
- **Sub-demotion live warning (active member) — softened:** the effective target becomes **`max($current, min($earned, 1))`** (1 = TL1). So an active member who has earned TL1 **graduates to TL1** (escaping the TL0 hold), the warning still **CAPS** promotion at TL1 (no TL1→TL2+ while a warning is live), and it **never demotes** (the `max($current, …)` keeps a TL2/TL3 member at their current level rather than dropping them).

The diagnostic mirrors the engine (no drift): `freezeReason()` reorders to (demotion → non-active status → warning) and reports the warning case as **"promotion capped at TL1 by N live warning point(s)"** ONLY when the member has earned past TL1 (otherwise the warning isn't what holds them, so it returns null and `below threshold` / `eligible → promoted to TL1` shows instead).

**Consequences.** A warned but otherwise-eligible member escapes new-user moderation at TL1 promptly (with the Branch-2 eager recompute on post approval, immediately). Spammers/abusers are unaffected: the TL1 cap keeps a warned account out of the higher-trust tiers (link/image/elevated gates) until the warning expires, and the hard demotion still applies at the threshold. No migration; no `acl_entries`/resolver change.
...
### ADR-0093 — Design-Polish Program (post-1.0) (2026-06-22)
**Status: Proposed — design-polish program (built + gated 2026-06-22 across 6 branches off `main`, unmerged; flagged for owner ratification). Lifted from `docs/product/design-polish-adrs-DRAFT.md` (renumbered draft 0092→0093 after confirming ADR-0092 is taken by the trust-warning freeze); the draft is deleted by v1.x R2.**

**Context.** Two external IPS audits found NovFora's design *foundation* is strong (semantic-token system, dual-mode dark, density scaling, AA floor, Theme-API contract, an `<x-ui.*>` set) but **uneven** — a CSS class leak shipped the audit's worst-rated defect (the `.novfora-prose` height cap on rendered posts, fixed as M0), editor richness is thin vs. incumbents, and empty/loading/error states aren't systematic. The owner's directive: **form weighted equally with function.**

**Decision.** Run a post-1.0 **Design-Polish Program** as a parallel, tracked track in four pillars — (1) mature the token set into a documented, gap-filled `<x-ui.*>` library (adds `table` + `skeleton`; the rest already exist), (2) ACP navigability/feel (persistent sidebar shell, the standard table, one form system, quick-links), (3) member-experience polish, (4) a curated-rich, keyboard-first, **independently-designed (clean-room)** rich-text editor as the flagship. It threads through the functional milestones 1.1 (member UX) / 1.2 (ACP v4) / 1.3 (admin tooling); every change is **tokens-only** (auto dark + density) and passes the deterministic gates **including the a11y page gate**; "done" includes visual + a11y, not just green tests (`design-polish-program-2026-06-22.md` §5).

**Consequences.** Polish becomes reusable + regression-guarded rather than per-surface luck; ACP v4 + member-UX consume the same components; **no new product scope** beyond the editor **attachment subsystem (ADR-0094)**, which is apex. Model routing: mostly Sonnet (view/CSS/component sweeps via Explore sub-agents); the editor upload path is **Fable @ max**.

### ADR-0094 — Editor attachment subsystem (apex) (2026-06-22)
**Status: Proposed — apex slice of the Design-Polish Program (built + gated 2026-06-22 on `claude/polish-p4-attachments`, unmerged; the mandated per-finding adversarial verify-then-refute review is on record — 0 HIGH, 1 MEDIUM fixed in-session, 1 LOW closed by v1.x S1). Lifted (renumbered draft 0093→0094).**

**Context.** The composer (`content-editor.blade.php` + the TipTap island) supports only **single, image-only, click-to-pick** upload; the flagship needs the screenshot's **drag-and-drop, multi-file attach zone with a max-size readout**. File upload from the internet is an **untrusted-input boundary** — the apex rung per `CLAUDE.md`. P5.1 shows attachment handling already exists (the soft-deleted-post download gate; the importer path-traversal fix), so this **extends, not duplicates** it.

**Decision.** An additive/reversible `attachments` table (uploader · polymorphic/`post_id` · disk · random stored path · original name · mime · size · checksum); a **permission-gated, throttled upload endpoint** that enforces a size cap (surfaced in the UI), an **extension allowlist + MIME sniff** with mismatch rejection, **image re-encode + EXIF strip + dimension clamp** (with a pre-decode total-pixel budget against decompression bombs), **inert storage + forced safe `Content-Disposition`** for non-images, and sanitised stored filenames (reject `..`/schemes); a **serve endpoint** authorising on parent-content visibility (**private-club no-leak**) and mirroring the **soft-deleted gate** (uploader/moderator only); **tier-aware storage** (local disk Baseline / S3·MinIO Enhanced — no daemon dependency on Baseline); draft→post association on publish + an **orphan-prune cron** (`novfora:attachments:prune`, short-mutex, restore-skipped). The TipTap side adds a file-card node (extending the `EmbedNode`/`SpoilerNode` pattern) and generalises the existing image drag/paste handlers to all files; render stays canonical-JSON → server sanitise allowlist. **A per-finding adversarial verify-then-refute review is mandatory before merge** (MIME/extension confusion, SVG/script, size/zip-bomb, traversal, serve IDOR, private-club leak, unauth, rate-limit bypass, association hijack), recorded like the P5.1/RH-4 reviews.
...
### ADR-0101 — Manual trust-level + reputation editing on the per-member admin view (apex) (2026-06-22)
**Status: Accepted — built + gated + merged (v1.x F2); the mandatory adversarial verify-then-refute review returned GO (0 open HIGH); the 1 MEDIUM + 3 LOW it confirmed were fixed before merge and re-verified.**

**Context.** Staff could set a member's group and ban/warn them from the per-member ACP view (ADR-0096), but not directly set a **trust level** or **adjust reputation** — both are legitimate manual staff actions (correct a mis-graded member, recognise/penalise standing). Trust is load-bearing: trust levels ARE ACL groups (tl0–tl4) that the permission engine resolves, so a manual change is a permission change. Reputation feeds `GroupAutoPromoter` thresholds. Doing either by hand must reuse the engine — never bypass it.

**Decision.** Two new **additive** capability keys — `members.trust.manage`, `members.reputation.manage` — in the catalog (Administration-tier, granted to the `administrator` preset so `PermissionSync` propagates them on upgrade; only a full admin can mint them into a role). Both actions are engine-gated (`canDo`) AND rank-guarded (`ActorRank` — no acting on an equal/higher-ranked member; fail-closed); they are **not** anti-spam-gated (a deliberate staff override). Trust set reuses **`TrustLevelManager`**: the trust-group swap + the established **`MembershipCache`** flush (version-bumped on change, so the resolver re-keys and the inspector verdict flips immediately), and marks a new reversible `users.trust_locked` flag so `evaluate()` treats the level as a **sticky floor** — the cron recompute never structurally demotes below it, the auto-engine may still promote ABOVE it, and a serious-infraction total still HARD-demotes to TL0. Reputation routes through the **existing ledger**: `ReputationService::adminAdjust()` makes the change's **AuditLog row the `award()` source**, so the signed delta flows through the idempotent insert + atomic denorm increment exactly like a reaction award (and `recomputeFor()` sums it) — never a side write. Every change is audited (`user.trust.manual_set` from→to/actor/reason; `user.reputation.admin_adjusted` delta/from→to/reason).

**The apex review (GO, 0 HIGH) — fixed before merge:**
- **MEDIUM — atomic, row-locked trust-group swap.** The group swap was non-transactional, so a manual set racing the cron recompute (or a second admin) could transiently leave a member in TWO trust groups, which the resolver reads as the most-permissive UNION of both levels. Fix: a shared `swapTrustGroup()` wraps the detach-all-trust + attach-one + cache flush + denorm write in a `DB::transaction` with a `lockForUpdate` on the user row (closing the inherited race in `manualSet()` AND `setLevel()`); a test asserts exactly one trust group after a set.
- **LOW ×3 (fixed):** the reputation audit's from→to now reads the prior total under the same lock (truthful under concurrency; the denorm + ledger were already exact); the trust audit writes `actor_id` from the explicitly-passed `$actor` (not ambient `auth()`), matching the reputation path; and `trustReason` gained the server-side `max:500` cap its siblings carry.

**Consequences.** Manual trust/reputation is a first-class, capability-gated, fully-audited staff action that reuses the engine's seams (membership cache, reputation ledger) rather than forking them. A manual trust set is a durable admin intent the cron cannot silently undo; an admin who configures a reputation-threshold custom group accepts that a manual rep adjustment feeds the same auto-promoter as organic rep (bounded by the rank guard + the admin-only key). One additive reversible migration (`users.trust_locked`); baseline-runnable.

### ADR-0102 — Apply the "Indie Web Hearth + Nova" brand to the app (self-hosted fonts; AA-retuned tokens) (2026-06-23)
**Status: Accepted — built + gated (v1.x brand). Per `brand/NovFora-Brand-Guidelines.md`; brand finalised in ADR-0026.**

**Context.** The app shipped on a placeholder indigo/slate palette with a system-ui-only type stack (zero font files, no CDN — a privacy + offline-build value). The locked NovFora brand (ADR-0026) is *Indie Web Hearth + Nova*: **Nova Blue leads** the UI, **Ember Amber** is the warm signature, teal is demoted to **success-only**, **hearth-cream** light + **warm-obsidian** dark, editorial **serif** headings. The semantic design-token NAMES are the THEME-API public contract (ADR-0009 / `docs/THEME-API.md`) — values may change, names may **not**.

**Decision.**
- **Token VALUES only** swapped in `resources/css/app.css` (the 3 blocks: `:root` light + both dark blocks); every THEME-API name preserved (the `A11yTest` `--novfora-accent` / `.skip-link` / `:focus-visible` literals stay intact). Added a NEW semantic token **`--ember` / `--ember-ink`** (+ `bg/text/border-ember` utilities) for the warm signature, since `--accent` is now Nova Blue. Dark elevation shadow tint warmed (`rgb(0 0 0 …)` → `rgb(10 6 2 …)`).
- **Self-hosted fonts.** Inter (body/UI) + Newsreader (serif headings + the `.font-display` wordmark) bundled locally via `@fontsource` → woff2 enters the Vite build, served from our own origin. **No CDN / third-party request at runtime** — the privacy property is preserved. **Accepted tradeoff:** the byte-reproducible offline build now includes the font binaries (was zero-font-files). The marketing site may still load them from Google Fonts.
- **Group-colour AA re-tune.** The warmer cream surfaces (worst = `--surface-sunken #ebe3d3`) have less headroom than the old cool `#eef1f7`, dropping three `GroupColor` light hexes below AA 4.5:1. teal `#0f766e→#0e7069`, blue `#2563eb→#245fbb`, violet `#7c3aed→#7a39e8` were nudged darker to restore the contract (`GroupColorTest` surfaces updated to the brand values; dark hexes unchanged — they pass comfortably). All other token text/UI pairings verified ≥ AA in both modes (deterministic WCAG audit).
- **Ember scope.** Ember is the SIGNATURE (logo, wordmark, hero "home", sparing warm highlights) — **not** a status colour and **not** the primary action. Its light value `#b5731f` meets AA-large (3:1) for the display/logo use it actually has; the primary action stays Nova Blue (`--accent`, ≥ 4.5:1).
- **Icons / PWA.** New constellation-bubble favicon (`public/favicon.svg`) for tabs; a full-bleed obsidian app icon (`public/icons/novfora.svg`) regenerated to PNG (16/32/180/192/512 + maskable-512) and a 48px `favicon.ico`. The layout favicon links / apple-touch-icon, the `<meta name=theme-color>` (now per colour-mode: cream / obsidian), and the PWA manifest `theme_color`/`background_color` (Nova Blue / obsidian) were rewired to the brand. Subpath-aware per ADR-0078 (icon srcs still via `asset()`); `PwaTest` apple-touch assertion repointed to `icon-180.png`.
- **Standalone pages.** The pre-bundle pages that can't use the Vite CSS (installer, error, maintenance, offline) carry the brand palette **inline**, and the ACP accent-hint copy updated ("built-in indigo" → "built-in Nova Blue"). `ThemeApi::editableTokens()` blank-defaults updated to the brand light values.

**Consequences.** The app reads as NovFora in both light and dark; AA holds for group + semantic tokens in both modes (verified). Token NAMES unchanged → **no THEME-API break** (child themes/widgets unaffected). Reversible; baseline-runnable; `npm run build` re-emitted with the fonts bundled.

### ADR-0103 — Embed API / SSI / web components for external sites (U7, apex) (2026-07-01)
**Status: Accepted — built + gated (U7 / NOV-106); spike memo at `docs/product/spike-u7-memo.md` (GO); adversarial verify-then-refute review on record (PROJECT-STATE).**

**Context.** External sites want to surface a community's activity (latest topics, board stats) on their own pages — the classic phpBB "SSI" ask, modernized. Every prior art either leaks (session-riding widgets), invites clickjacking (frame-anywhere pages), or hard-depends on a JS runtime. This is an untrusted-embedding boundary: the consuming page is not ours, the request's origin is not ours, and nothing about the embed may widen what an anonymous visitor could already see.

**Decision.** One server-rendered core with three consumption modes, all **guest-visible content only**, feature **OFF by default** (`embeds.enabled` setting, config-backed by `NOVFORA_EMBEDS`):
- **Endpoints** (`routes/embed.php`, mounted `embed/v1` via `bootstrap/app.php then:` — deliberately OUTSIDE the `web` group): `GET /embed/v1/w/{widget}` (self-contained HTML for iframe/SSI; zero scripts, inline styles, `<main>` + skip link, light/dark/auto) and `GET /embed/v1/d/{widget}.json` (the same payload as JSON). **Stateless:** no session, no cookies, GET-only — a framed embed carries no authority. Widgets v1: `topics` (optionally forum-scoped) + `stats`; the element names, attributes, URL shapes, and JSON shape are a **semver'd public contract** (`version: 1` in every JSON body; `public/embed/embed.js` v1.0.0).
- **Per-embed allowlist** (`embed_sites`, written only by the audited `EmbedManager`): name + exact **origin** (`scheme://host[:port]` — paths/queries/credentials/wildcards/non-http(s) refused at normalization) + a public, rotatable, revocable **site key** (`emb_` + 40 random; the "scoped read-only token" — scope = render guest widgets for that origin, nothing more; full keys never enter the audit log, suffix only). The HTML response sets its own `Content-Security-Policy: … frame-ancestors 'self' <origin>` (the `SecurityHeaders` middleware's fill-only-when-absent seam); the JSON response grants `Access-Control-Allow-Origin` to exactly the registered origin (no `*`, no credentials) + `Vary: Origin` unconditionally. NO global CORS layer is introduced — the rest of the app stays closed.
- **No-leak fence:** every read goes through the proven guest principal — `User::guest()->canDo('forum.view', …)` + `VisibleForumIds::for(guest)` + `Forum::clubContentVisibleTo(null)` — with **404 on every denial** (feature off, bad/rotated key, unknown widget, invisible forum: indistinguishable; the FeedController/Sitemap idiom). Visibility verdicts run LIVE per request; only the resulting viewer-independent payload is cached (60s, plain arrays — RH-9).
- **Abuse bounds:** named `embed` rate limiter (per-IP, config-tunable, cache-backed = DB on Baseline / Redis on Enhanced per ADR-0011), payload caps (limit clamp 1..20), `Cache-Control: public, max-age=60` + nosniff + strict referrer policy on every response.
- **Web components:** `public/embed/embed.js` — hand-authored, dependency-free classic script (no Vite entry, no Node at runtime; the "prebuilt assets" rule holds). Defines `<novfora-topics>` / `<novfora-stats>`; derives the API base from its own `src` (subdirectory-safe); renders payload fields as **text nodes only** inside a closed Shadow DOM; a fetch failure renders nothing (never breaks the host page).
- **ACP:** Plugins → Embeds (`⚡embeds` Livewire SFC; `admin.access` + staff-2FA re-asserted per action) — register/toggle/rotate/remove sites, flip the master switch (writes through `Settings`, audited), copy-paste iframe + web-component snippets; the key is displayed in full exactly once.

**Accepted residuals (documented in the spike memo):** legacy CSP2-less browsers fail-open on `frame-ancestors` — tolerable because the content is public and action-free; site keys are public-by-design (revocation + origin-binding is the control, not secrecy); a forum made private can persist in an embed for ≤60s of cache TTL (tighter than the sitemap's 1h / feeds' 15min precedents). Authenticated/member-scoped embeds are explicitly out of scope — that tier would need real scoped tokens and a consent story; the seam (per-site `widgets` allowlist, versioned contract) is reserved.

**Consequences.** External sites get first-class, secure embeds with no daemon, no Node, no new dependency — Baseline-native (Enhanced only speeds up the cache). One additive reversible migration (`embed_sites`). New surfaces joined the WCAG page gate; adversarial fixtures (forged origins, oversized/malformed params, cross-origin JSON, private-forum/club probes, rotated keys, rate-limit breach, statelessness) live in `tests/Feature/Embeds/`.

**Adversarial review (verify-then-refute, 5 attack lenses × per-finding verify — GO, 0 open HIGH/MEDIUM after fixes).** Confirmed + fixed before the fixes commit:
- **The one substantive finding (rated up to MEDIUM), fixed:** the *board-wide* topics widget resolved the guest-visible forum set INSIDE the `Cache::remember` closure and keyed the cache on a constant (`…topics.all.{limit}`), so a just-restricted forum's topic titles kept serving for the rest of the 60s TTL — inconsistent with the *scoped* path, which fences live and 404s immediately. Fix: resolve `VisibleForumIds::for(guest)` LIVE (outside the cache) and fold a signature of the visible set into the cache key, so a restriction changes the key → a fresh, correctly-fenced query on the next request. A regression test (`EmbedSecurityTest`) proves the drop happens within the TTL without flushing the fragment cache. (The ≤60s *downstream* `Cache-Control: public` window remains the documented, deliberate residual — tighter than the sitemap's 1h / feeds' 15min.)
- **LOW hardening applied:** `normalizeOrigin` now re-validates the parsed host against a strict hostname/bracketed-IPv6 charset, rejecting `;`/`,`/control/raw-UTF-8 bytes that `parse_url` otherwise keeps in the host (admin-only input, no widening was possible — the appended CSP directive was inert — but it closes the input-validation gap at the source); the `embed` rate limiter buckets IPv6 sources by their /64 prefix.
- **Refuted (no change):** `?site[]=` array param (Symfony `InputBag` throws a benign 400 before reaching the typed resolver, no oracle); control-char origins (`parse_url` replaces C0/DEL with `_`); IDN/trailing-dot origins (fail closed — never match a real browser origin); the web-component `href` (every URL is server-generated via `route()`, no `javascript:` reachable); the `Cache-Control: public` window (the cached bytes are guest-public by construction).

### ADR-0104 — Plugin install-from-zip + signature/trust gate (U17, apex) (2026-07-01)
**Status: Accepted — built + gated (U17 / NOV-115); spike memo at `docs/product/spike-u17-memo.md` (GO); adversarial verify-then-refute review on record (PROJECT-STATE).** (ADR-0103 = U7 embeds, allocated in parallel on its own branch; confirm both land on merge.)

**Context.** Modules shipped install-only-from-disk (ADR-0031): an admin copies `modules/<vendor>/<name>/` up by FTP, then `ModuleManager::install()`. H3 closed the trust-guardrails flag (consent gate, integrity hash, quarantine, kill switch) but explicitly left a **package signature + a distribution channel** as "documented future enhancement, not built". U17 closes that: upload an untrusted `.zip` through the ACP and have it install ONLY if it is safe to extract AND authentic. Two orthogonal, untrusted-input boundaries — a hostile archive and an unsigned/tampered package.

**Decision.**
- **Hostile-zip hardening (`App\Modules\Packaging\ArchiveGuard`).** Never `ZipArchive::extractTo` on an untrusted archive (that is safe only for `RestoreService`'s self-produced backups). Instead iterate entries and write each with a bounded, streamed copy to a path proven inside the staging root. Consequences: zip-slip/traversal (names with `..`, leading slash/backslash, drive letter, null byte rejected; resolved target re-checked under root), **symlink escape structurally impossible** (we only ever `file_put_contents` regular files — a symlink entry becomes an inert regular file, so nothing exists for a later entry to traverse through), zip-bomb (entry-count + per-file + total-uncompressed + per-entry compression-ratio caps, read from the central directory THEN re-enforced against the ACTUAL streamed bytes so a lying header can't slip a bomb), and an extension allowlist. All caps config-driven (`novfora.modules.zip.*`).
- **Signature / trust gate (`App\Modules\Packaging\PackageSignature` + `module_trust_keys`).** A signed package ships a `module.sig` at its root: the base64 **ed25519** signature (bundled `ext-sodium`, constant-time verify) of the package's canonical digest — a sha-256 over the path-sorted (name, length, bytes) of every OTHER file (the `packageHash` shape, minus the sig). Verified against the admin-managed trusted-key registry (`App\Modules\ModuleTrustKeys`, audited add/toggle/remove; fingerprint = sha-256(raw key)). **Reject by default:** a present-but-invalid signature is ALWAYS refused (tamper / untrusted signer), even under the dev `allow_unsigned` policy; an unsigned package is refused unless `allow_unsigned` (env, loud, dev-only) is on. Fail-closed if `ext-sodium` is absent (every package reads as unsigned → rejected).
- **Atomic, reversible commit (`App\Modules\Packaging\ModuleInstaller`).** Extract to `storage/app/module-staging/<rand>/`; validate the manifest (via the existing `ManifestValidator` — the slug comes from `module.json`, never the upload filename); verify the signature; only then `rename(staging, modules/<slug>)` and hand off to the existing audited `ModuleManager::install` (fresh) or `::upgrade` (existing dir, operator-confirmed, backs up + restores on failure). Any failure rolls the files back out and **quarantines** the archive to `storage/app/module-quarantine/` with an audited reason (`module.zip_install.rejected`); success audits `module.zip_install.accepted`. Installs DISABLED — enabling still needs the H3 full-trust consent.
- **ACP + tooling.** A new `⚡module-install` Livewire SFC on the Modules page (admin.access + staff-2FA per action) for the upload + the trusted-key registry. `novfora:module:sign` is the author-side recipe (keygen + write `module.sig`); the server only ever verifies.

**Out of scope (unchanged):** a real PHP execution sandbox (full-trust-on-enable remains the documented model — U17 secures authenticity + integrity, not runtime isolation); a remote marketplace / fetch-over-network (this is upload-a-file — no SSRF surface; the signature format + trust registry are the seam a future marketplace reuses).

**Consequences.** Operators can install signed third-party plugins through the browser with no FTP, no daemon, no queue, no new Composer dependency — Baseline-native, bounded by the archive caps. One additive reversible migration (`module_trust_keys`). Adversarial zip fixtures (traversal, absolute path, symlink, bomb, bad signature, untrusted key, too-many-entries, disallowed type, invalid/absent manifest, install-over-existing) live in `tests/Feature/Modules/PluginZipInstall*Test.php`.

**Adversarial review (verify-then-refute, 5 attack lenses × per-finding verify — GO, 0 HIGH, 0 MEDIUM).** Every security-class hypothesis was REFUTED against the committed code: no signature/trust bypass (the digest covers the whole staged tree minus `module.sig`; a file added after signing changes it; present-but-invalid always rejected), no write-outside-staging / traversal, no bomb bypass (the per-file + total byte caps re-checked on ACTUAL streamed bytes fully bound extraction regardless of a lying header), no 2FA bypass (a restricted per-user-grant admin still isStaff-gated correctly — refuted), quarantine unbounded-growth deemed non-security. The 10 confirmed items were all **LOW / robustness**, and the cheap ones were applied so the ADR's reversibility claim is honest:
- **Upgrade atomicity + backup-dir hygiene (fixed):** the two file-moves ran BEFORE the try, so a failed swap-in (cross-fs + ENOSPC) stranded the live module with no restore; and `freshDir()+'-backup'` orphaned an empty staging dir each upgrade. Now the whole swap is inside the try (restore on any failure) with a `finally` that cleans the backup, and the backup path is computed directly (no orphan).
...
### ADR-0106 — Username history + admin change/revert (U8) (2026-07-02)
**Status: Accepted — built + gated (v1.2 / NOV-97).**

**Context.** No username write path existed post-creation: `username` was set once at registration / social signup / import / install and never again — the ACP member screen rendered it read-only, and user-side editing stays deliberately off (BUG-019: `display_name` is the user-editable identity). But admins legitimately need to rename members (impersonation cleanup, offensive handles, support requests), and a rename of an identity field without an audit trail or an undo is a moderation-accountability gap. Complication: `username` is the route key (`User::getRouteKeyName`), so any change instantly 404s the member's old `/users/{username}` URL.

**Decision.**
- **`username_history` table** (singular, the `audit_log` append-only-log precedent): `user_id` FK cascadeOnDelete, `old_username`, `new_username`, `changed_by` (nullable, no FK — mirrors `post_revisions.editor_id`), `reason` (500), timestamps, composite `[user_id, created_at]` index. Additive + reversible, `hasTable`-guarded like the house migrations.
- **Service chokepoint, not model events:** `App\Members\UsernameService::change()` / `::revertTo()` — every "record history + audit" feature here (post edits, bans, trust overrides, reputation adjustments) is an explicit transactional service, never a `User::updating` hook, so the guards stay colocated with the write. Both methods run inside one `DB::transaction` with the target's current name re-read under `lockForUpdate` (racing changes serialize; from→to stays truthful — the `ReputationService::adminAdjust` shape), validate with the exact registration rule (`alpha_dash, min:3, max:30, unique` ignoring the target's own row — `CreateNewUser`), snapshot the history row BEFORE the overwrite (the `post_revisions` ordering), update via `forceFill`+`save`, and write an explicit-actor `AuditLog` row (`user.username.changed` / `user.username.reverted`, changes `from/to/reason` — the `TrustLevelManager::manualSet` idiom). A no-op (same name) is a validation error, not a silent success.
- **Revert fails loud.** A revert writes back a name that WAS the member's — but if someone else has since claimed it, the unique rule throws a `ValidationException` surfaced in the ACP. Never auto-suffix on revert (the `_n` dedup trick used by import/social *signup* would silently rename to something the admin didn't ask for, defeating the revert). `revertTo` also refuses an entry belonging to a different member.
- **Capability: reuse `users.manage`** — the existing per-member account/PII key (email visibility, sessions, force-reset) already models "may administer this member's account identity"; no new capability key is minted. The SFC actions re-assert the staff-2FA gate + `users.manage` + the no-self/rank guard (`assertActionable`) exactly like `banUser()`; no sole-owner guard (renaming is not access-stranding).
- **ACP UI:** a "Username" card on `⚡admin.members.manage` right after the Account summary — current `@name`, change form (optional reason), and the history list (old → new, actor, date) with a per-row "Revert to this" **ghost**-variant button behind `wire:confirm` (restorative tone, the liftBan precedent — not danger). Mutating controls render only when the target is actionable (the Trust-level card's outranked fallback).
- **Accepted consequence — route identity:** a change 404s the old profile URL immediately; revert exists to fix mistakes. The `Redirect`/`LegacyRedirects` 301 mechanism (importer-only today) is deliberately NOT wired to username changes this pass — serving redirects for renamed members is a separate product decision (privacy: it advertises the rename) tracked for later, not an oversight.
...
### ADR-0107 — CAPTCHA driver completion (hCaptcha/reCAPTCHA, fail-closed) + opt-in Gravatar (U18) (2026-07-02)
**Status: Accepted — built + gated (v1.2 / NOV-98).** (ADR-0105/0106 allocated on parallel v1.2 branches; confirm all land on merge.)

**Context.** The CAPTCHA abstraction (ADR-0007 §2.5) shipped with one external driver (Turnstile) and a documented promise that "hCaptcha/reCAPTCHA would be near-identical modules implementing this same contract". U18 finishes the driver set and adds the long-requested Gravatar avatar fallback. Two boundaries are in play: the driver-verification path receives **untrusted input** (a client-supplied token verified against a third-party endpoint), and Gravatar is a **privacy** boundary (a member-email derivative leaving the site).

**Decision.**
- **New drivers are FAIL-CLOSED.** `HCaptchaProvider` (POST `https://api.hcaptcha.com/siteverify`, field `h-captcha-response`) and `RecaptchaProvider` (reCAPTCHA v2 checkbox; POST `https://www.google.com/recaptcha/api/siteverify`, field `g-recaptcha-response`) mirror Turnstile's structure (`Http::timeout(4)->connectTimeout(2)->asForm()`, `verify()` never throws, `available()` = secret configured, `challenge()` returns ONLY type/field/site_key — never the secret) **except** the outage posture: a provider 5xx or connection failure returns **false**. At an untrusted-input boundary an unverifiable token is a failed challenge, never a pass. This does not lock registration out: the manager still degrades an *unconfigured* provider to Q&A, the honeypot/StopForumSpam/velocity/new-user-moderation layers still apply independently, and a legitimate user can simply retry.
- **Turnstile's shipped fail-open is retained untouched** — flipping it is a behaviour change to a shipped, reasoned decision (its comment argues the layered defence) and is **flagged as an owner follow-up**, not silently resolved either way. The posture divergence is documented in both new drivers' `verify()` comments.
- **Keys/secrets follow the two-tier pattern:** env (`HCAPTCHA_SITE_KEY`/`HCAPTCHA_SECRET`/`RECAPTCHA_SITE_KEY`/`RECAPTCHA_SECRET`) → config (`novfora.antispam.registration.captcha.{hcaptcha,recaptcha}`) → optional encrypted ACP override (4 new `SettingDefinition`s mirroring the Turnstile pair; secrets `encrypted: true`, "leave blank to keep" placeholder semantics in the ⚡antispam SFC). Provider select now `qa|turnstile|hcaptcha|recaptcha|none`.
- **CSP allowlist (both `policy` and `strict_policy`, kept in sync):** `script-src` + `frame-src` gain `challenges.cloudflare.com`, `js.hcaptcha.com` + `*.hcaptcha.com`, and `www.google.com` + `www.gstatic.com`. This **also fixes a latent shipped gap**: the default CSP's `script-src 'self' 'unsafe-inline' 'unsafe-eval'` never allowlisted `challenges.cloudflare.com` ('unsafe-inline' only permits inline script *bodies*, not a cross-origin `src=`), so the Turnstile widget loader — and its iframe, absent from `frame-src` — was silently CSP-blocked on a default deployment. Verified before fixing; all three providers' widgets now load under both policies.
- **Gravatar, privacy-fenced (default OFF).** New `members.gravatar_enabled` bool (group `members`, surfaced in the ⚡members-directory SFC behind the amber privacy callout, modeled on the `antispam.external_content_optin` fence). `App\Support\Gravatar::url()` only *computes* `https://www.gravatar.com/avatar/{md5(lower(trim(email)))}?s={size}&d=mp` — **no server-side HTTP call, ever**; the member's browser fetches the image at render time. Hooked into the single avatar seam (`<x-ui.avatar>`): uploaded `avatar_path` always wins → Gravatar only for a real user with an email when the setting is ON → initials fallback unchanged; the guest/deleted-author branch never resolves a Gravatar. The Settings singleton's per-request memo (RH-9) keeps the many-per-page reads at one cache hit. `img-src` already allows `https:` — no CSP change needed for the images.
- **Progressive enhancement holds:** QA/null remain the Baseline default (`NOVFORA_CAPTCHA=qa`); every external driver degrades to Q&A when unconfigured; nothing new is required on the Baseline tier.

**Consequences.** Operators choose among three external CAPTCHA providers from the ACP with encrypted secrets and no secret ever reaching the register page (tested via `assertDontSee`); the fail-closed boundary is pinned by dedicated `Http::fake` tests (success/failure/5xx/connection-exception/empty-token-no-call) in `tests/Feature/AntiSpam/CaptchaVerifyTest.php`; Gravatar is a zero-server-call, admin-consented fallback pinned by `tests/Feature/Members/GravatarTest.php` (`Http::assertNothingSent`). No migration; no new dependency; reversible by config. **Owner follow-up:** decide whether Turnstile should also become fail-closed for posture consistency.
### ADR-0108 — SEO polish: pending-topic fence, dynamic robots.txt, profile OG, author-search link (U20) (2026-07-02)
**Status: Accepted — built + gated (v1.2 / NOV-99).**

**Context.** The topic page was the only surface with a full canonical/OG/JSON-LD set; profiles had no canonical or OG at all; nothing emitted `og:site_name` or a default meta description; and two correctness problems sat underneath: (1) a topic whose opening post is pending moderation (the topic inherits its OP's state, ADR-0007 §2.4) was directly reachable at `/topics/{id}-{slug}` and at its Atom feed by anyone with `forum.view` — guests included on a public board — leaking its title and OP excerpt through `<title>`, breadcrumbs, `og:*`, JSON-LD, and the feed's `<title>`, even though every *listing* correctly hides pending topics; (2) the checked-in static `public/robots.txt` shadowed the dynamic `/robots.txt` route on Apache/Baseline (`.htaccess` rewrites to the front controller only when the request is NOT an existing file), so the route — the only source of the runtime `Sitemap:` URL, correct under subdirectory installs — was dead code, and the passing route test gave false confidence (Pest always dispatches through the router, never through Apache's static-file check).

**Decision.**
- **Pending-topic fence (shipped first, its own commit).** `TopicController::show` 404s a non-approved topic for everyone except its author and viewers with `topic.moderate`. **404, not 403** — the FeedController/sitemap no-disclosure idiom: a forbidden viewer can't distinguish "held" from "gone". The fence runs AFTER the club/`forum.view` gates (no new permission oracle) and BEFORE the canonical-slug 301 (the title-derived slug can't leak through a redirect). Even for the author/moderator carve-out, the ENTIRE SEO head block (canonical, description, OG, twitter, feed link, JSON-LD) is suppressed until approval, and the OP-excerpt description query is skipped for non-approved topics and sources only approved posts — query-neutral on the approved path (the moderate check moved earlier, not duplicated). `FeedController::topic` 404s a non-approved topic's feed outright, before its cache. `rejected` topics are soft-deleted on reject and already 404 via `trashed()` — deliberately not double-handled.
- **Dynamic robots.txt only.** `public/robots.txt` is DELETED: with no static file, Apache falls through to the front controller and the route serves `User-agent/Allow` + the runtime `Sitemap:` line (subdirectory-install aware). A test pins the file's absence so it can't silently come back.
- **Profile OG (lite).** Canonical (tab-param-free), `og:type=profile`, `og:title` "{display name} (@username)", `og:url`, twitter summary card, and a SYNTHESIZED aggregate-only description ("Member since {month year} · {N} posts" — join date + the `post_count` the members directory already exposes publicly). `og:image` = the member's avatar when `avatar_path` is set (a server-set, forceFill-only path; absolute via `url(Storage::url(...))`); no image tag otherwise. NEVER `signature_html` or post content — a member's posts may live in forums/clubs the viewer cannot see. No profile JSON-LD this pass.
- **Layout seams.** `layouts/app.blade.php` now always emits `og:site_name` from the existing wordmark fallback chain (site wordmark → `config('app.name')`), and supports an OPTIONAL `$metaDescription` passed via `@extends` data (exactly like `$title`): non-empty → one `meta name="description"` + `og:description` pair. Wired on the forum index (from the ACP `general.site_description`, empty → tags omitted), trending, and clubs index. Deliberately NO unconditional global description — pages that `@push` their own would double-tag.
- **Find-content-by-user.** The profile Posts tab (capped at 20 visible posts) links to `route('search.index', ['author' => username])` (`dusk="profile-all-posts"`) — the author search facet already existed end-to-end, permission-fenced; this is UI affordance only, zero backend change.

**Out of scope (deliberate):** tag/club OG blocks; a purpose-built 1200×630 social-card `og:image` asset (no site-wide default image is emitted); JSON-LD beyond the existing topic `DiscussionForumPosting`; and the pre-existing gap that `<title>`/PWA-manifest read `config('app.name')` directly and ignore the ACP `general.site_name` setting — flagged as a follow-up, not changed here.

**Consequences.** Pending content can no longer be scraped or shared from its own URL/feed during the moderation window; robots.txt is correct on every tier including subdirectory installs; profiles and the three index pages become share-friendly without widening any visibility boundary. New fences are pinned by `tests/Feature/Seo/PendingTopicFenceTest.php` (guest/member 404, author/moderator carve-out without the head block, feed 404 → all restored on approval) and `tests/Feature/Seo/SeoLeakTest.php` (private-club topic 404s before any OG; club-private posters keep aggregate-only profiles; single-description-tag guard).
