...
/**
 * Spike P2 — `php artisan hearth:deliverability:digest-run`. One assembler tick: self-heal stuck runs, then
...
{
    protected $signature = 'hearth:deliverability:digest-run';
    ...
    {
        if (! config('hearth.deliverability.enabled') || ! config('hearth.deliverability.digest.enabled')) {
            $this->info('Digest pipeline is dormant (hearth.deliverability.digest disabled).');
