...
class NotificationController extends Controller
{
    ...
    public function open(Request $request, string $id): RedirectResponse
    {
        ...
        // future emitter (or imported data) can never turn this into an open redirect. The base match uses a
        // trailing-slash boundary (base+'/'), NOT a bare prefix — a bare `str_starts_with($url, url('/'))`
        // would admit `https://novfora.com.evil/…`, `…@evil`, `…competitor.com` (no host delimiter). The
        // rooted-relative branch rejects `//host` and `/\host` (protocol-relative forms some browsers honour).
        $url = (string) ($notification->data['url'] ?? '');
        $base = rtrim(url('/'), '/');
        $rootedRelative = str_starts_with($url, '/')
            && ! str_starts_with($url, '//')
            && ! str_starts_with($url, '/\\');
        $sameOrigin = $url !== '' && ($rootedRelative || $url === $base || str_starts_with($url, $base.'/'));
